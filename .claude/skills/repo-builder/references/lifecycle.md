...
Two more travel as exclusive choices, where adopting both is the defect. `_config.yml` and `.nojekyll` are the GitHub Pages pair: `.nojekyll` turns off the Jekyll processor `_config.yml` exists to configure, so a repository holding both publishes a site whose configuration nothing reads. `.github/release.yml` and `CHANGELOG.md` are the release-notes pair: `scripts/release` publishes the changelog section when that file exists and generates notes only when it does not, so `release.yml` adopted beside a changelog is never consulted. Neither combination fails anywhere — both render, both validate, and the file that does nothing looks exactly like the file that does.
