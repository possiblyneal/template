...
The payload ships `docs/agents/`, so `/setup-matt-pocock-skills` is skipped by default; `references/generate.md` step 6 gives the cases where it still runs and the ordering that holds either way. Where it does run, invoke it with the Skill tool rather than answering its questions yourself or reproducing what it writes.
