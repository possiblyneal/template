...
**`scripts/CLAUDE.md` diverges between the two trees by one paragraph, on purpose.** The payload copy is the root copy minus the Work Guidance paragraph telling an editor to mirror a change into `src/base-repo/scripts/` — a generated repository has no second tree to mirror into. Everything else is identical, so `diff -r scripts apps/github-repository-template/src/base-repo/scripts` reports that one file and nothing else; a second difference is a drift to fix, in whichever tree is behind.
