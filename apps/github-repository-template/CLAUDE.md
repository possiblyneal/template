...
**Dependabot only ever bumps the root.** `.github/dependabot.yml` scans `/` and `/.github/actions/*`, and nothing it lists reaches `apps/`, so a bot pull request bumping a pinned action updates the root copy and leaves the payload pinned to the old revision. Mirror the bump into `src/base-repo/.github/` on the bot's own branch before merging it. Nothing in the files themselves says the trees disagree, since both are valid YAML and both pass actionlint, so `scripts/github-parity` is what reports it: it fails the commit and the gate until the bump is mirrored. It refuses the divergence rather than repairing it, which is what turns each root bump into a complete one.

**`src/addon-adoption.json` is root-only and stays that way**, one of the two deliberate exceptions to the rule above; `scripts/CLAUDE.md` below is the other. It describes `src/repository-addons/`, which never reaches a generated repository, so mirroring it into the payload ships an index of files that are not there. It is also a sibling of that directory rather than a file inside it, because anything inside is an addon to copy. The in-file guidance carried by the addons themselves is not deduplicated against it: that guidance serves someone adopting a file by hand, in a repository that has no manifest to read.

**A payload file may exist to serve an addon**, where the unadopted state is a no-op rather than a failure. `src/base-repo/scripts/system-packages` reads `.github/system-packages.txt`, which never ships: without it the script stats one path and exits 0. That is what satisfies the test "The payload is what is useful on day one" sets below — adding a file to the payload asserts its condition holds for every generated repository — since an absence that costs nothing asserts nothing. A shipped file that errors without its addon fails every generated repository instead, which is the line.

**The `check-json` exclude diverges between the two `.pre-commit-config.yaml` copies on purpose.** `devcontainer.json` is JSON with comments — the dev container schema sets `allowComments` — and both the addon and any copy adopted into a generated repository carry their guidance in them, so the strict-JSON hook has to be told to skip the file. The path differs because the file lives in different places: root excludes the addon at `apps/github-repository-template/src/repository-addons/.devcontainer/devcontainer.json`, the payload excludes `.devcontainer/devcontainer.json` at a destination root. A normalized diff of the two configs shows five divergences and no others: this `exclude` line; the comment above it, since root's explains an addon path and the payload's a destination path; the two root-only hooks `addon-adoption` and `reference-assertions`, which check paths that never reach a generated repository; and the `rulesets` comment, which names this template at the root and cannot at the destination. The `github-parity` hook is not among them: the check ships in both trees and reports `not-applicable` where there is no payload, so a repository that later grows one is guarded without a sixth divergence to maintain.

**`scripts/CLAUDE.md` is root-only.** It is this repository's own contract for `scripts/`, and the payload ships no copy — a generated repository builds its `CLAUDE.md` tree from the skeleton at `src/base-repo/CLAUDE.md` rather than inheriting prose it did not write. So `diff -r scripts apps/github-repository-template/src/base-repo/scripts` reports that one file as only-in-root and nothing else; a second difference is a drift to fix, in whichever tree is behind.
...
- `src/base-repo/apps/app-name/` is replaced during generation, and the payload says nothing about it. `apps/repo-builder/src/references/generate.md` step 8 owns the rename: the placeholder is moved to the first derived deployable rather than copied, so it cannot survive generation.
- **The payload ships no directory whose only content is its own name.** `apps/app-name/` carries `.unit.json` and `src/` and nothing else; root `libs/`, `tests/`, and `docs/specs/` and the unit's `tests/` and `docs/specs/` are not shipped. Each was a third statement of what the Layout rules in `src/base-repo/CLAUDE.md` and `docs/github_repository_structure.md` already make — one that carries no reason, cannot be checked, and is free to drift from both. Shipping `tests/` empty at the root *and* at the unit additionally posed the scope question with nothing in either to answer it, and `docs/specs/` contradicted the shipped `docs/agents/issue-tracker.md`, which puts specs on the tracker. Every one remains a valid placement; the structure doc records them as conventions rather than shipped directories. `tmp/`, `docs/plans/`, and the `.claude/` subdirectories keep their `.gitkeep` — the first is gitignored and cannot survive empty otherwise, the second is where plan mode writes, and the third is the surfaces bullet below. `generate.md` step 8 replicates the two-entry skeleton, so widening it here means widening it there.
...
**`.github/dependabot.yml` lists only actions and pre-commit**, and that is not an oversight. An entry naming a manifest the repository does not have fails with `dependency_file_not_found` rather than being skipped, and a template cannot know which ecosystem a clone will use — listing six guarantees five broken entries in every one. Nobody copies an entry out of this file by hand either: every `/repo-builder` flow that writes it derives one entry per manifest the destination tracks and appends it to these two, re-deriving the whole set each run, so the entry survives an update by being rebuilt rather than by being carried. The rule is "Dependabot entries follow the manifests present" in `apps/repo-builder/src/references/lifecycle.md`, and the file's own header states the manifest-to-ecosystem mapping for whoever is reading the file rather than the rule.
...
One check reads this directory semantically: the `addon-adoption` pre-commit hook runs `apps/repo-builder/src/scripts/tests/test_addon_adoption.py`, which fails when `src/addon-adoption.json` and `src/repository-addons/` disagree about which files exist, when an addon sits at a path `src/base-repo/` already ships, when an entry declares a slot token no longer present in its file, or when any addon ships as a zero-byte file. It runs on every commit rather than on a path filter, because `git rm` of an addon is the case a filter cannot see. It does not check that a described region is described *well* — only that it exists.

The payload's behavior is verified where it lands: by `src/base-repo/scripts/tests/*-test` once a repository is generated, and by the `repo-builder` evals under `apps/repo-builder/src/evals/`.
