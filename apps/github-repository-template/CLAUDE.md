...
- `src/base-repo/apps/app-name/` is replaced during generation. The payload says nothing about it: the rename belongs to the `repo-builder` skill's wayfinding step, where the boundaries are derived, and a payload paragraph restating it costs every generated repository tokens to read a rule it can no longer act on. `.claude/skills/repo-builder/references/lifecycle.md` step 4 owns it — the placeholder is moved to the first derived deployable rather than copied, so a directory that only looks like a convention cannot survive generation.
