...
- `src/base-repo/apps/app-name/` is replaced during generation, and the payload says nothing about it. `.claude/skills/repo-builder/references/generate.md` step 8 owns the rename: the placeholder is moved to the first derived deployable rather than copied, so it cannot survive generation.
