...
`scripts/github-parity`: Refuses a divergence between a repository's own `.github/` and the copy a template payload ships to generated repositories, comparing every path in either tree from the Git index. `dependabot.yml` is the one named content exception, since a payload copy must not carry an ecosystem entry for a manifest it does not ship; a payload file the root deliberately dropped is excused only where `generation.features` in `.repo-template.json` records the omission, which needs `jq` and is reported `unavailable` without it. Reports `not-applicable` in a repository with no payload, which is every repository generated from one. Called by `scripts/check` and by pre-commit on every commit.
