...
Every job in all four workflows takes its runner from a `RUNNER` repository variable and falls back to `ubuntu-latest` when it is unset, so a repository redirects its own CI with `gh variable set RUNNER --body self-hosted` rather than by editing a workflow the template owns and colliding with it on the next update. Hosted is the default deliberately: a self-hosted runner on a public repository lets any fork's pull request run arbitrary code on the host, so the configuration that carries that exposure is never one a repository gets by accident. `codeql.yml`'s `analyze` job reads the variable only where its matrix asked for Linux, leaving Swift's `macos-latest` entry alone, and a self-hosted machine is not a drop-in for the hosted image — `docs/adrs/0004-select-the-runner-through-a-repository-variable.md` has the costs.
...
`docs/agents/`: How the engineering skills read the repository — `issue-tracker.md` names where issues live and how to work them, `triage-labels.md` the label vocabulary, `domain.md` the glossary and ADR layout. Shipped rather than collected, for the reasons `docs/adrs/0002-ship-agent-skill-configuration-in-the-payload.md` records; `apps/repo-builder/src/references/generate.md` step 6 confirms these files and creates the labels they name. Editable by hand afterwards. Read by path rather than through any index, which is what keeps all three reachable in a repository whose `CLAUDE.md` it has not written yet.
