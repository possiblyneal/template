...
`doctor` treats an uninstalled pre-commit hook as a missing tool. `.pre-commit-config.yaml` is tracked, but the hook it describes lives in `.git/hooks/`, local to a clone and never committed, so a fresh clone has the config and none of the enforcement. Nothing surfaces that on its own: commits succeed exactly as before while the configured hooks, the secret scan among them, never run. `doctor` therefore fails when the config is present but either the tool or an installed hook is missing, and stays silent when there is no config to honor. It checks every hook file the config asks for, and decides which to require by reading the config rather than by assuming, so a repository that drops the commit-message check is not reported as broken for having dropped it. That reading lives in `scripts/libs/precommit.sh`, shared with the session-start hook that installs what is missing, and it parses the hook list out of both of YAML's list forms — a pattern matching only the inline one goes quiet the moment the file is reformatted, and quiet here means no hook and no complaint.
