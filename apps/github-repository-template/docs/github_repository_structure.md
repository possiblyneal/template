...
Every workflow stays as simple as possible: the bulk of the code lives in `scripts/`, so it can run independently of pushing to GitHub.
...
`session-start.sh` is a `SessionStart` hook that prints the current branch, uncommitted changes, and recent commits as session context, restates the read-before-edit obligation and names the rule file holding the full documentation contract, and runs `pre-commit install` when the config is present but a hook it asks for is not yet installed. It runs on `startup`, `resume`, `clear`, and `compact`; the last two matter most, since `/clear` and compaction are where an agent loses its orientation and this re-establishes it. The install step exists because a fresh clone has `.pre-commit-config.yaml` and none of the enforcement until someone runs the install, and the agent opening a session is the earliest moment that reliably happens. It tests for every hook file the config asks for, not just `pre-commit`: a clone predating the `commit-msg` hook already has the first one, so a check for that alone would report the clone as set up and leave the message check permanently uninstalled. Which hooks those are is `scripts/libs/precommit.sh`'s answer rather than a second reading of the config here, for the reasons kept with that file.
...
`doctor` treats an uninstalled pre-commit hook as a missing tool. `.pre-commit-config.yaml` is tracked, but the hook it describes lives in `.git/hooks/`, local to a clone and never committed, so a fresh clone has the config and none of the enforcement. Nothing surfaces that on its own: commits succeed exactly as before while the configured hooks, the secret scan among them, never run. `doctor` therefore fails when the config is present but either the tool or an installed hook is missing, and stays silent when there is no config to honor. It checks every hook file the config asks for, and decides which to require by reading the config rather than by assuming, so a repository that drops the commit-message check is not reported as broken for having dropped it. That reading lives in `scripts/libs/precommit.sh`, shared with the session-start hook that installs what is missing.

`repo-settings check` reports the settings under Repository settings that a clone cannot enforce for itself: squash merging, push protection, and an active branch ruleset. Squash merging is reported first and unconditionally, because it is offered on every plan while the other two are not, and the checks for those return early when the plan does not offer them. Both are remote state, so nothing in the tree reveals whether they are on, and both back a guarantee made here — the pre-commit hook keeping commits off `main` is absent in a fresh clone, and the CI secret scan runs after a push rather than before it. Keeping this inspection out of `doctor` makes `doctor`, `check`, and `dev` fast and usable offline rather than coupling ordinary local work to GitHub authentication and API availability.
...
\*\*`scripts/libs/precommit.sh`\*\*: Which git hooks `.pre-commit-config.yaml` asks for, and which of them a clone lacks. `doctor` reports that gap and `.claude/hooks/session-start.sh` closes it, so both need one answer; the two copies this replaced had already drifted apart in how they tested for a hook file, and a startup hook quietly disagreeing with the command that reports on it is worse than either answer alone. It is separate from `detect.sh` because that library owns language-specific decisions and a git hook is present whatever a repository is written in — there is nothing to detect.

It parses the hook list rather than matching one line of it. `default_install_hook_types` is valid as an inline list, as an inline list wrapped across lines, and as a block list, and a formatter turns the first into the second without the key changing. A pattern that reads only the first line finds nothing or half a list, reports no hook beyond the default, and both callers stop requiring the commit-msg hook — with no output to say so, which is the failure this file exists to prevent rather than an inconvenience it works around. The config path is a parameter and the clone is the working directory, since only one of those is a path a caller chooses.
...
\*\*`scripts/tests/`\*\*: `capabilities-test` asserts that every language present is dispatched to every check. It is the one property nothing else reports on: a language dropped from dispatch produces a shorter green run, not a failure. Fixtures are scaffolded into temp directories and asserted against `CI_DRY_RUN=1`, which prints the check and language pairs `ci` would run and runs none of them, so the result comes from wiring alone and is identical on any machine with no toolchain installed. `health-checks-test` stubs `gh` to prove that `doctor` and `dev` stay offline and that hosted inspection happens only through `repo-settings check`. `adr-index-test` asserts that the generated ADR index converges and that pre-commit actually invokes the hook, including on a deletion — the script passing while nothing runs it is the failure a script-level test cannot see. `commitlint-test` drives real commits through a real `commit-msg` hook, since a hook that was never installed and a linter with no rules both look exactly like a working setup from the config alone. `precommit-hooks-test` covers the layer underneath it: that the hook list is read correctly out of each of YAML's list forms, which is the part that fails without saying anything. `libs/harness.sh` holds the assertion counting and the temp-directory teardown they share, while each suite keeps its own `fixture`, since they scaffold different worlds and only the counting and the removal are common; it is sourced rather than executed, like the libraries under `scripts/libs/` and for the same reason. These live beside the scripts they test rather than in the root `tests/`, which is reserved for the project's own cross-app tests. Every `*-test` runs from `scripts/check` and directly from `ci.yml` before toolchain setup.

`scripts/release` does the one part of publishing a template can know: it checks the tag against `vX.Y.Z`, reads the matching `## [x.y.z]` section out of `CHANGELOG.md`, runs `scripts/ci`, and creates the GitHub release with that section as the body. It never chooses the version — the tag does that, and the shape check is the only place in the repository that enforces semantic versioning rather than merely documenting it. Release-note automation built on Conventional Commits would replace most of this, and is rejected for the reason the changelog entry below gives: notes derived from commit messages are addressed to the wrong reader, and the tool would have to be chosen per ecosystem in a repository that does not yet know its language.

Two of its refusals are the point. An absent or empty section for the tag fails the release, so a version cannot be published without having been cut in the changelog first — the drift that makes a changelog worse than none is caught before the release exists rather than noticed after. And `scripts/ci` must be present and executable, so a green release always means the full gate ran. Because the script both verifies and publishes, the workflow holds `contents: write` while `scripts/ci` executes the project's own toolchains. Splitting verification into a read-only job would close that window at the cost of running the entire gate twice, which is the trade this makes in the other direction.

Artifacts stay project-specific, but they are no longer a blocker: a release carrying notes and no binary is a complete release, and the script marks where a build step goes. The release workflow installs the same toolchains as the CI workflow, since `scripts/ci` fails when a manifest is present and its toolchain is missing. The tag comes from an explicit argument before `GITHUB_REF_NAME`: a `workflow_dispatch` run is dispatched against a branch and its ref is not the tag being released. The workflow passes that input through the environment rather than `${{ }}` interpolation, which would place a user-supplied string directly into a shell command.
...
The distinction is worth stating because the two folders age differently. `scripts/` stays flat, extensionless, and readable without a build step, and a compiled program dropped into it arrives with a manifest that the root-level checks then have to be told to ignore. A shell script filed under `tools/` costs less, but it removes the reason `tools/` exists and leaves a repository where neither name predicts what a file is.

The payload ships the convention and not the directory, because an empty `tools/` asserts that every generated repository will grow a built helper, and most never do. Recording the rule here is what keeps a compiled program out of `scripts/`; the folder standing empty adds nothing to that.

\*\*`tmp/`\*\*: Git-ignored scratch space for temporary files, downloaded artifacts, and dumped logs, holding a `.gitkeep` so the directory survives being empty. It exists because `.claude/hooks/ask-outside-repo.sh` prompts on any write resolving outside the repository root, which leaves an agent needing scratch space with two bad options: interrupt the session for permission to write to the system temp directory, or write into the tree, where the file lands in `git status` and eventually in a commit. This is the third — inside the root so the hook allows it, ignored so it can never be committed. The hook ships on day one, so the directory it implies does too.
...
\*\*`CHANGELOG.md`\*\*: A per-version record of what changed, in Keep a Changelog 2.0.0 format, pinned by the link in the file's own header so the convention it follows cannot drift as that page changes. Ships with a placeholder `<owner>/<repo>` in the reference link at the bottom: versions are Markdown reference links resolved there, each pointing at its comparison with the one before, so a generated repository replaces the owner and name once and every later version links to the exact diff. GitHub Releases already is a per-version record — each release carries a tag, a date, and notes — so the question is not whether to keep a changelog but whether to keep a second copy of it in the tree. It is not a question of what the entries can say: a release body can be hand-written and a file can be generated, so both surfaces hold whatever the project is willing to write.
...
That is why it ships rather than waiting for the first release. The mechanism is the `## [Unreleased]` section: entries accumulate there one pull request at a time and are cut into a version when the tag is made, so the file has to exist before the first change worth recording, not before the first tag. Started at tag time it has nothing to carry and the history it would have held cannot be reconstructed. Where the changes are made by agents that stops being a matter of degree: a session's context is deleted when it ends rather than fading, so the party that made the change is not available at tag time to describe it, and the pull request is the only window in which the entry can be written at all. The empty section is also the prompt — absent the file, nobody asks whether a change needed a line.

The file reads in the other direction too. A session starting cold reconstructs what the project has lately been doing from it far more cheaply than from `git log`, which is uncurated and spends context in proportion to how much happened. That value does not wait for a release either, and it is the reason the file earns its place in a repository that never publishes one. A smaller point in its favour is that it lives in git, while release notes live in GitHub's database and do not survive a move to another host.

Against it, a hand-maintained file drifts from the tags the moment a release is cut without updating it, and a changelog that disagrees with the releases page is worse than neither. The resolution is to keep the file canonical and draw the release from it rather than maintaining both: each version sits under a predictable `## [x.y.z]` heading, so `scripts/release` extracts that section and posts it as the release body, and one edit produces both. It refuses to publish a tag whose section is missing or empty, which turns the drift into a failed release rather than a releases page disagreeing with the tree.

Two things depart from 2.0.0 on purpose. It reserves the judgment of what is notable for a human, on the grounds that a fluent changelog nobody has read is now cheap to produce. Here that judgment sits with whoever writes the entry, because it is the judgment that decides whether there is an entry at all — deferring it to a later human means the change reaches `main` with the question still open, and by then the session that made it is gone. The entry arrives in the pull request diff instead and passes the same review the change itself does. And it assumes a previous tag when linking `Unreleased`, which a repository with no releases does not have — the payload points it at the commit list until the first tag, which shows the same set of changes a comparison would. Neither the specification nor Keep a Changelog's own changelog covers that case, so it is an extrapolation rather than the documented answer.
...
shellcheck runs with `-x`, which follows `source` into `scripts/`. Without it every script sourcing the detection library fails on SC1091 rather than being analyzed against what it sourced. The sourced libraries are the exception to the shebang rule above — everything under `scripts/libs/`, and `scripts/tests/libs/harness.sh`. They are sourced, never run, so they carry no shebang and no executable bit, and the `.sh` extension is what identifies them as shell.

commitlint is the one hook here that runs at `commit-msg` instead of `pre-commit`, because it reads the commit message, which does not exist when the others run. That stage is a separate file in the git hooks directory and `pre-commit install` writes only the `pre-commit` one unless told otherwise, so the config carries `default_install_hook_types: [pre-commit, commit-msg]`. Without that line the command reports success, the hook is never installed, and every message goes unchecked — the uninstalled-hook failure again, one level further in. It also means `pre-commit run --all-files`, and therefore `scripts/check`, never exercises commitlint; `scripts/tests/commitlint-test` drives real commits instead.

`@commitlint/config-conventional` is pinned inside `additional_dependencies` and has to be bumped by hand. Dependabot's pre-commit ecosystem updates a hook's `rev` and never reads that list, so an unpinned entry would follow whatever npm serves that day and a commit rule could change with no diff to review. The hook is `language: node`, so pre-commit provisions an isolated Node environment for it under its own cache. That environment is invisible to `scripts/libs/detect.sh`, which decides a repository is JavaScript by the presence of `package.json` — none of this adds one, so no repository starts dispatching JavaScript checks because its commit messages are linted.

\*\*`.commitlintrc.yaml`\*\*: The commit message rules. Messages follow Conventional Commits — `type(optional scope): subject`, a blank line, then the body — which makes the shape machine-readable rather than merely conventional, and gives the changelog a signal: a commit typed `feat` or carrying a `BREAKING CHANGE:` footer announces that an entry is probably owed. It remains a signal. Notability is judged per pull request, and a changelog assembled mechanically from commit types is the commit log with extra steps.

The file extends `@commitlint/config-conventional` and overrides two rules, `body-leading-blank` and `footer-leading-blank`, raising each from warning to error. The Conventional Commits specification states both as MUST, and both are correctness rather than style: without the blank line git reads the body as a continuation of the subject, and a `BREAKING CHANGE:` footer stops being machine-readable at exactly the moment it matters most. Everything else is the stock preset. YAML rather than `commitlint.config.mjs` so the file needs no `package.json`, for the reason above, and the existing `check-yaml` hook validates it on every commit as a side effect of the format.

Nothing local can check a squashed pull request's message, since that message is the pull request title and is written in GitHub's web interface. A merge commit's subject is generated by GitHub and already sits on commitlint's default ignore list, so merging rather than squashing keeps the whole convention enforceable from the one place a hook can reach.
...
Every condition below turns on a person whose terms the repository does not set. An AI collaborator is not one: it is bound by the `CLAUDE.md` chain, which it reads on every session, rather than by the files here. So building with agents instead of alone triggers none of these — it is not the second party any of them are written for. It does change what the repository owes its own future sessions, which is why `CHANGELOG.md` and `docs/lessons.md` ship in the payload instead of appearing in this list.
