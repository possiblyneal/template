...
The template ships `documentation.md`, holding the contract governing `CLAUDE.md` files: read the nearest one before editing, update the owning file after meaningful changes, the shape and section order of a child file, and the closeout pass. It carries no `paths:` frontmatter, so it loads in every session. Scoping it to Markdown would be the obvious economy and is the wrong one: the obligation is to read the chain *before* editing anything, and a rule that arrives when a `.md` file is already open arrives after the moment it governs. Rules with `paths:` frontmatter are also dropped at compaction and do not return until a matching file is read again.
