...
\*\*`.claude/settings.json`\*\*: Overrides for global `settings.json`. The template fills `hooks` with the entry above and ships the remaining containers empty, so a new project sees the available sections without inheriting rules: `permissions` \(`allow`, `ask`, `deny`, `additionalDirectories`\), `env`, `sandbox`, `attribution`, `enabledPlugins`, `modelOverrides`, and `skillOverrides`.
