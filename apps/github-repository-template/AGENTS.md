...
- `.mcp.json` ships empty, and `.claude/` ships empty `agents/` and `workflows/`, so a new project sees those surfaces without inheriting rules. `.claude/skills/` and `.claude/output-styles/` are the exception: they ship curated content — the `agentmemory`, `mattpocock`, and `personal` skill families and the `minimal` output style — as the payload's source of truth. The whole `.claude/` tree and `CLAUDE.md` are gitignored in a generated repository, so — like `.env` — they arrive in its working tree but not its history, and are force-tracked here to ship at all.
