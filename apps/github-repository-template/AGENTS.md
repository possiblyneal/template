...
- `src/base-repo/apps/app-name/` is renamed during generation. The root `AGENTS.md` of the payload carries the Placeholders section instructing that rename; a placeholder that only looks like a convention gets kept rather than replaced.
...
- `.mcp.json` ships empty, and `.claude/` ships empty `agents/` and `workflows/`, so a new project sees those surfaces without inheriting rules. `.claude/skills/` and `.claude/output-styles/` are the exception: they ship curated content — the `agentmemory`, `mattpocock`, and `personal` skill families and the `minimal` output style — as the payload's source of truth. The whole `.claude/` tree and the root `CLAUDE.md` are gitignored in a generated repository, so — like `.env` — they arrive in its working tree but not its history, and are force-tracked here to ship at all. The ignore is anchored to the root: a nested `CLAUDE.md` is the `@AGENTS.md` shim that loads its folder's contract, so it stays committable there.
- `.claude/rules/` ships `documentation.md` only. `changelog.md` sits in `src/repository-addons/` alongside the `CHANGELOG.md` it governs, because a rule about maintaining a file the repository does not have is a contract an agent cannot satisfy. The two travel together or not at all.
...
None. `src/base-repo/AGENTS.md` is payload content addressed to a generated repository, not a child contract for this one.
