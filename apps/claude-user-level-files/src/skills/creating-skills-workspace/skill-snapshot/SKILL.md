...
Post a short "Interview complete" summary with these 4 fields before creating or editing SKILL.md.
...
- **description**: The primary triggering mechanism. Write as routing logic, not marketing copy — answer: when to use, when not to use, outputs. Write in third person. Front-load trigger words. Claude tends to undertrigger, so lean slightly pushy. Include explicit negative cases ("Don't use when...") to reduce misfires.
...
Before writing any SKILL.md body content, post an EDD decision:
- `evals_in_scope: yes|no`
- Rationale
- Initial eval prompts (if yes)

Only after this summary may you write SKILL.md body content.
...
- `description` is 1-1024 chars and covers what/when
...
- `scripts/quick_validate.py` — Validate SKILL.md against spec
