...
- `src/scripts/preflight.py` — validates and describes generate, update, adopt, and retrofit inputs before any flow acts. Retrofit shares the git helpers and none of the manifest ones: every check in `load_provenance` reads a record a retrofit has not written yet.
