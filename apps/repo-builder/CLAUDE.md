...
- `src/scripts/preflight.py` — validates and describes a flow's inputs before it acts. Its `retrofit` subcommand lands ahead of the flow itself: `src/references/` holds no `retrofit.md` yet, so nothing routes to it. Retrofit shares the git helpers and none of the manifest ones: every check in `load_provenance` reads a record a retrofit has not written yet.
