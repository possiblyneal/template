...
- **Retrofit:** The destination has content and no `.repo-template.json`. It is a repository that grew without the template, so the payload is landed over work somebody is already shipping rather than into an empty tree. The flow is written as far as everything that must be true before anything is published — the candidate, its working hooks, the unit map the operator confirms, the moves that follow from it, the record, the collisions, and the bar; a run stops there and reports, so route here knowing the hosted-write gate and publishing are still unwritten.
...
- `update` validates provenance, repository identities, a clean destination, strict ancestry, and the bounded template delta. A delta path the record overrides comes back marked `overridden` with its recorded reason, and out of the managed tally, so the flow does not reconcile a collision that was settled once already.
