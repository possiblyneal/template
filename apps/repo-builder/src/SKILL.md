...
- `update` validates provenance, repository identities, a clean destination, strict ancestry, and the bounded template delta. Each delta path carries a `destination_state` of `unmodified`, `modified`, or `absent`, so reconciliation reads content only for the paths that were edited on both sides. A delta path the record overrides comes back marked `overridden` with its recorded reason, and out of the managed tally, so the flow does not reconcile a collision that was settled once already.
