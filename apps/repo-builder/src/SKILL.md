...
- **Retrofit:** The destination has content and no `.repo-template.json`. It is a repository that grew without the template, so the payload is landed over work somebody is already shipping rather than into an empty tree. Route here rather than to generate: generate never inspects a destination, so an occupied one looks identical to an empty one and the operator gets a flow that cannot see what it is writing over.
...
A generate whose own destination turns out to hold content — a repository someone pushed to between step 4 and step 11, or one a resumed generate re-enters — stays a generate, but its collisions are decided by hand and it never resolves one by deleting. `references/generate.md` carries those rules; read them before writing to a non-empty destination. A destination that held content before the flow started is the retrofit above instead, and the two are told apart by which side created the repository.
