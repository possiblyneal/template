...
5. Record every collision the destination's version won in `generation.overrides`, one `{path, reason}` entry each, as [Overridden paths](lifecycle.md#overridden-paths) specifies. A collision the payload's version won landed the payload file and is an ordinary copy; it takes no entry.
