...
- Destination hooks after merge (retrofit): installed with `--allow-missing-config`, `core.hooksPath` pinned local to <path>, verified by <the refusals observed> | n/a (merge declined), under [Working hooks in the destination after a merge](lifecycle.md#working-hooks-in-the-destination-after-a-merge)
