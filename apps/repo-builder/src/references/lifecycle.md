...
- **A path the flows replace outright can never be an entry.** `.gitignore` is the one, under [retrofit's Step 6 — Reconcile the destination's layout](retrofit.md#step-6--reconcile-the-destinations-layout), which is where the rule is stated and where the first record is written: the payload's version lands and a destination rule the payload does not cover is reported for the operator to settle rather than re-added. [The payload's mode travels with the payload's content](#the-payloads-mode-travels-with-the-payloads-content) is what carries it to every other flow landing that file, naming the ignore file among the paths replaced whole. A file that always lands is not a file that was never written. Ownership cannot catch the mistake, since the path is managed and the rules admit the entry, so `preflight.py` refuses it by name — at the next update or adopt, which reads the record, rather than at the retrofit that wrote it, which has no record to read yet. That is the point the harm would otherwise land, so the refusal arrives in time, but a retrofit writing this entry gets no warning at the time of writing and the run itself has to not write it. The cost of letting it through is that the entry protects nothing and discards everything: an update skips a delta path listed here, so every later payload change to the ignore file is dropped on the strength of a collision that never happened, and no check reads a file that stopped being updated.
...
One path inside the prefix this does not settle on its own. `dependabot.yml` is replaced like the rest and then gains the entries [Dependabot entries follow the manifests present](#dependabot-entries-follow-the-manifests-present) derives from the destination's manifests. That is not a merge of the destination's version, which is what the guarantee above is protecting — the file is still written whole from template-owned logic, and the destination supplies evidence rather than content. `codeql.yml` needs no such rule: it lands like any other path and decides per run, as [Code scanning lands everywhere and decides at run time](#code-scanning-lands-everywhere-and-decides-at-run-time) sets out.
...
A destination that wants no scanning at all deletes `codeql.yml` itself and records `"codeql": "omitted-by-choice"` under `generation.features`. That is the destination's choice rather than this skill's, and the record is what tells a later flow the file was omitted rather than lost — an omitted file and a deleted one look identical on disk.
...
**A workspace file is not a manifest.** `go.work` and `settings.gradle.kts` say where the modules are; they are not what Dependabot reads. A destination with a root `go.work` and one `apps/<name>/go.mod` takes a single `gomod` entry at `/apps/<name>` and none at `/`. Whether an entry at `/` would *also* reach that module is Dependabot's own discovery behaviour rather than a fixed rule, and it moves: `go.work` discovery was merged into `dependabot-core` in `dependabot/dependabot-core#14909` on 2026-05-05, which dates the change to the engine rather than its rollout to every repository, and a root holding a `go.work` and no `go.mod` is the narrow case where an entry at `/` had nothing to read before it. Derive from the manifests regardless — that is the form this rule can state without tracking a per-ecosystem feature it does not control. It is also the hand-enumerated per-module list that upstream change was written to retire, and the reason it is still right here is that the whole set is rebuilt every run: what made the enumeration fragile was keeping it in step by hand. **A cargo workspace is one manifest set, not one per member.** A root `Cargo.toml` holding `[workspace]` earns the single `cargo` entry at the workspace root, and a member listed in its `members` earns none of its own, although both files are named `Cargo.toml` and the rule above would otherwise give each one. Cargo's file fetcher reads `members` out of the root manifest and fetches every member's `Cargo.toml` as an updatable file rather than a support file, and the root `Cargo.lock` travels with the bump, so the root entry already proposes the members' bumps and moves the lockfile they need. A member entry does not add coverage: it opens a second pull request bumping the same lines, and where the member declares its own versions rather than inheriting with `{ workspace = true }` the root manifest never enters that job, so the second pull request carries no lockfile to move at all. `dependabot/dependabot-core#12766` is that failure. The day a member inherits a dependency with `{ workspace = true }` it becomes reachable from a job of its own and the arrangement is worth revisiting; until then, derive one entry at the workspace root.

`scripts/doctor` reports every such member as unwatched regardless, because `dependabot_unwatched_manifests` compares literal directories and has no notion of a workspace. It reports rather than fails, and it is not a reason to add the entry — the check's own advisory names the exception.

**Read the manifests from the tracked files, with `git ls-files`.** Not from `has_<lang>` in `scripts/libs/detect.sh`, which answers only whether a *root* manifest is present, and not from a search of the working tree pruned by `DETECT_PRUNE_DIRS` — that list is `.git node_modules .build target .venv venv vendor tmp`, written for root-manifest detection rather than for a walk, and it does not name `.claude/worktrees/`, where this repository keeps gitignored checkouts of itself. A pruned search run here today returns two `pyproject.toml` that are not in the repository, and an entry naming one of them fails the whole run with the error this rule exists to prevent. What Dependabot reads is the pushed tree, so the tracked set is not an approximation of the right answer — it is the right answer. Exclude the payload under `apps/github-repository-template/src/base-repo/`, which is data destined for other repositories rather than a manifest of this one; it ships none today, and an entry derived from one it later ships would name a path no destination has.
...
- **The hooks directory is the clone's own, not a sibling in the scratch tree.** A candidate's `<hooksdir>` is disposable with the candidate; this one outlives the flow, and the scratch tree it would otherwise sit in belongs to this repository rather than to the destination. So `.git/hooks` is where it goes, and the key is pinned to it absolutely, because a relative value resolves against each worktree's own top and the clone has more than one.
