...
- **A path every flow replaces outright can never be an entry.** `.gitignore` is the one, under [The payload's mode travels with the payload's content](#the-payloads-mode-travels-with-the-payloads-content): the payload's version lands at every flow, and a destination rule the payload does not cover is reported for the operator to settle rather than re-added. A file that always lands is not a file that was never written. Ownership cannot catch the mistake, since the path is managed and the rules admit the entry, so `preflight.py` refuses it by name — at the next update or adopt, which reads the record, rather than at the retrofit that wrote it, which has no record to read yet. That is the point the harm would otherwise land, so the refusal arrives in time, but a retrofit writing this entry gets no warning at the time of writing and the run itself has to not write it. The cost of letting it through is the bullet below, read backwards: the entry does not protect the destination's version, it discards every later payload change to a file the destination is still carrying, and no check reads a file that stopped being updated.
...
Three things about that clone are ordinary rather than exceptional, and each decides part of the recipe:

- **It is not on the merged branch.** The operator was working somewhere when the flow began and is still there. Install from a detached worktree on `origin/<default-branch>`, which is where the config now is. `.git/hooks` resolves through the common directory, so a linked worktree installs into the clone itself and the operator's own checkout is never touched.
- **A `core.hooksPath` may be set globally**, for the reasons the section above gives. `pre-commit install` refuses while it is set at any scope, so mask it for the child process alone rather than writing to the operator's config: `GIT_CONFIG_COUNT=1 GIT_CONFIG_KEY_0=core.hooksPath GIT_CONFIG_VALUE_0= pre-commit install …`. Then pin `core.hooksPath` at `--local` scope to the clone's own absolute `.git/hooks`, absolute because a relative value resolves against each worktree's own top and the clone has more than one.
- **Branches in flight predate the config.** Every one of them lacks `.pre-commit-config.yaml`, and the shims exit non-zero on a missing config, so a plain install starts refusing every commit on work this flow does not own. Install with `--allow-missing-config`: those branches then pass through untouched and the default branch gets the full surface.

Pinning the key takes the clone off `.git/hooks`'s global replacement, so the same `<type>.legacy` chaining the section above describes applies here too — link whatever the operator's global hooks directory held before pinning, unless the shims the payload installs already do that hook's job, in which case say so in the report rather than chaining a duplicate.
