...
- Adopt neither writes nor reads an entry. It lands an addon the destination does not have, at the recorded commit, and resolves no collision, so it never meets an override. Generate and update are the flows this section binds; retrofit joins them where it resolves a collision of its own.
