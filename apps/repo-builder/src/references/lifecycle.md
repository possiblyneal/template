...
**The file's own check run is not available on the candidate.** GitHub validates `.github/dependabot.yml` and reports a check named for the path, but it posts that check on a push to the default branch and not on a pull request branch — a retrofit's merge commit carries `.github/dependabot.yml=success` against event `main`, and the pull request that produced it carried no such check at all. So do not poll for it with the rest: a flow that waits on this one waits on something that will not arrive, and then reports a gate it never saw. Verify the entries by parsing the file that was written — one entry per tracked manifest, each `directory` a directory that manifest is actually in — and say in the report that the check run is observable only after the merge, or sooner from *Insights → Dependency graph → Dependabot → Check for updates*. `dependency_file_not_found` is the failure this rule exists to prevent and the one it can still cause, and the parse is what catches it before the merge rather than after.
