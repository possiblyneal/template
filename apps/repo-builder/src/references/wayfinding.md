...
- **Nothing about where the map lives.** `/wayfinder` reads `docs/agents/issue-tracker.md` and charts against whatever that file records. The payload ships it at that path and `/wayfinder` reads it there, through no index, and [Generate](generate.md) step 6 confirms it and creates the labels it names, against a destination repository that already exists. Do not name an effort directory and do not override the choice. A map charted somewhere other than where the repository tracks its issues is a map nobody finds again, and on the ordinary GitHub answer the map's native blocking edges are what render the frontier in GitHub's own UI — the thing `/wayfinder` calls essential and the thing a directory of markdown files cannot do. Ordering the repository ahead of wayfinding is what buys this; the tracker doc resolves the repository from `git remote -v`, so a handoff before that repository exists has nowhere durable to chart to — and `gh issue create --label wayfinder:map` fails outright until step 6 has created that label.
