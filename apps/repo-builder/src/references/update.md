...
4. Reconcile only paths in the preflight delta:
   ...
   Check expiry over `generation.overrides` itself rather than over the delta, as [Overridden paths](lifecycle.md#overridden-paths) directs: read every entry, whether or not its path is in the delta. The case the rule exists for is a destination that deleted its own version while the template left the path alone, which is exactly the case no delta contains. Where the destination no longer holds its own version, the override has nothing left to protect: land the payload's copy and delete the entry from `generation.overrides`, both in this update's pull request. An override kept past the file it was protecting keeps the payload's version out for a reason nobody holds any more.
5. Classify every delta path as `applied`, `preserved`, `renamed/deleted`, `overridden`, or `conflicted`. Do not leave conflict markers. An `overridden` path was never written, so it is absent from step 6's copy proof and belongs to neither the copied set nor the authored surface; report it by name with its reason. A path whose override expired above is `applied`.
