...
4. Reconcile only paths in the preflight delta:
   ...
   A delta path listed in `generation.overrides` takes none of the four, and every entry is read for expiry whether or not its path is in the delta. Preflight has already done that reading: its `unmatched_overrides` list names every entry the delta does not reach and marks each `expired` or `unreached`, so an expired entry is dropped in this pull request rather than found by re-reading the record by hand. Both rules and the reasons behind them are at [Overridden paths](lifecycle.md#overridden-paths); this step does what that section says and adds nothing.
