...
4. Personalize the candidate:
   ...
   - render visibility and feature choices honestly. Keep `codeql.yml` for a private repository rather than omitting it: the workflow gates its own `analyze` job on visibility, so it records a skip and the reason instead of failing, and a repository that goes public later starts scanning without anyone remembering to restore a file. Record an omission under `features` only when deliberately stripping the workflow, which is a choice now rather than the private-repository default.
