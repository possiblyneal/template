...
- `scripts/tests/session-start-test` — exercises `~/.claude/hooks/session-start.sh`, the operator's global copy: the hook reports stale metadata without pruning it, and still prints the session context when `scripts/worktree-cleanup` is absent
