...
**Go capabilities run once per module, not once at the repository root.** `./...` matches only packages inside a module, and the root of a workspace is not itself in one: `go build ./...` there refuses the pattern outright and checks nothing, so a repository whose only module sits under `apps/` reported a green run over work nothing looked at. `go_each` names the directories to enter with `go list -m -f '{{.Dir}}'` — the main module for a plain `go.mod`, every `use` entry under a `go.work` — and runs the command inside each, treating a listing that fails or comes back empty as a failure rather than an empty success, since it means the module graph could not be read at all. `_capability_format_check_go` and `_capability_format_write_go` stay at the root because gofmt walks the filesystem rather than resolving a package pattern. `_capability_dev_go` still assumes the root and is knowingly left there: `scripts/dev` takes an app name, and resolving that name to a module directory is a behaviour question this does not answer.
