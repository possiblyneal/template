...
- Code scanning: **unavailable**, and `.github/workflows/codeql.yml` skips rather than fails. Its `detect` job reads repository visibility and gates `analyze` on `public`, writing the reason to the run summary. Confirmed the gate is the only thing holding it back: the same workflow analyzed three languages and uploaded cleanly on a public repository generated from this payload. Read `docs/lessons.md` before removing the gate.
- GitHub Actions: **verified**. `ci.yml`, `security.yml`, and `codeql.yml` all pass on `main`.
