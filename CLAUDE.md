...
- Branch rulesets: **active on `main`**, named `main`, with no bypass actors — an administrator disabling the ruleset is the only way past it. It requires a pull request, restricts the merge to a true merge commit, refuses deletion and non-fast-forward, and requires the three checks that report on every pull request whatever the repository's state: `CI`, `Security`, and CodeQL's `Code scanning enabled`, the one that fails when the scan cannot read the visibility it would otherwise stand down on. `Analyze python` is deliberately not among them; `apps/repo-builder/src/references/generate.md` has why a name from CodeQL's language matrix does not belong in a rule gating a branch. `required_approving_review_count` is `0`, because GitHub refuses self-approval with HTTP 422 and any higher number leaves a solo pull request unmergeable; `require_extra_approval_for_unattributed_changes` is off for the same reason, since it demands that unavailable approval of any commit GitHub cannot attribute to an account. The `code-reviewed` label and its `Reviewed-Head:` comment stand in for the approval. `strict_required_status_checks_policy` is off, which leaves a stale base to convention rather than to the ruleset: `/land` runs the full local gate when the base moved, and a merge made from the GitHub button does not. It lives at the remote, so no `--no-verify` reaches it and a fresh clone is covered before `pre-commit install` runs; `no-commit-to-branch` and `scripts/protect-branch` stay useful as the earlier, faster refusal — they refuse a commit made while HEAD is `main`, and a push whose destination ref is `main`, the case the first cannot see.
