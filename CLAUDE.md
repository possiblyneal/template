...
- Secret scanning and push protection: **enabled**. This repository is public, which is what made them available; GitHub offers neither on a private repository on this plan. Push protection refuses a push carrying a recognized secret at the remote, so it is the first guard here that no `--no-verify` reaches. gitleaks in `.pre-commit-config.yaml` still runs at commit and still sees formats GitHub's providers do not, so it is the earlier check rather than a redundant one.
- Branch rulesets: **available and not configured**. `scripts/repo-settings check` reports `rulesets not-applicable` because none is active, so `main` is guarded only by the two local hooks: `no-commit-to-branch` refuses a commit made while HEAD is `main`, and `scripts/protect-branch` refuses a push whose destination ref is `main` — the case the first cannot see, since `git push origin HEAD:main` from a feature branch never makes HEAD `main`. Both live in each clone and both yield to `--no-verify`, so a fresh clone has neither until they are installed.
...
- Code scanning: **enabled**. `.github/workflows/codeql.yml` no longer stands down: its `scanning` job finds the repository public and `detect` and `analyze` run rather than appearing on the checks list as skipped, covering actions and python with zero alerts. That stand-down path is still in the workflow and still correct, because the payload ships to repositories that are private; `docs/LESSONS.md` has why it goes green rather than failing there. `generation.features` is empty, since nothing is omitted by choice.
