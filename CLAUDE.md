...
- Code scanning: **unavailable**, so `.github/workflows/codeql.yml` runs its `scanning` job, finds this repository private, and stands down — green, with a warning annotation and `detect` and `analyze` on the checks list as skipped. This repository has no static analysis coverage; the skipped jobs and the annotation are where that reads. `generation.features` is empty, since nothing is omitted by choice any more. The workflow uploaded cleanly on a public repository generated from this payload, so visibility is the only thing holding it back and scanning starts by itself if this repository goes public. `docs/LESSONS.md` has why it stands down rather than failing.
