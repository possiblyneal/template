...
- Secret scanning and push protection: **enabled**. This repository is public, which is what made them available; GitHub offers neither on a private repository on this plan. Push protection refuses a push carrying a recognized secret at the remote, so like the ruleset below it is enforced where no `--no-verify` reaches. gitleaks in `.pre-commit-config.yaml` still runs at commit and still sees formats GitHub's providers do not, so it is the earlier check rather than a redundant one.
- Branch rulesets: **active on `main`**, named `main`, with no bypass actors. It requires a pull request, refuses deletion and non-fast-forward, and requires `CI` and `Security` to pass. It is the first guard on `main` that lives at the remote rather than in a clone, so unlike `no-commit-to-branch` and `scripts/protect-branch` — which refuse a commit made while HEAD is `main`, and a push whose destination ref is `main`, the case the first cannot see — no `--no-verify` reaches it and a fresh clone that never ran `pre-commit install` is covered by it anyway. Those two hooks stay useful as the earlier, faster refusal, not as the guarantee.
  - The two CodeQL analysis checks are deliberately not required. Their names come from the language matrix `detect` builds, so requiring `Analyze python` would block every pull request the day the matrix stops producing it — the repository going private again, or Python leaving the tree. `CI` and `Security` are named by their workflows and always report.
  - `required_approving_review_count` is `0` because GitHub refuses self-approval with HTTP 422, so any higher number makes a solo pull request unmergeable. The `code-reviewed` label and its `Reviewed-Head:` comment are what stand in for approval here.
  - `strict_required_status_checks_policy` is off, so a pull request need not be rebased onto a moved `main` before merging. `/land` already compares the merge base against `origin/main` and runs the full local gate when the base moved, which is the same guarantee without the forced update.
  - Enforcement is proven rather than assumed: a `git push --no-verify` of a commit straight to `main` is refused with `GH013`, naming both the pull-request rule and the two required checks.
