...
- Entries go in the root `CHANGELOG.md`, never in `apps/github-repository-template/src/base-repo/CHANGELOG.md`. The payload's copy is the empty scaffold a generated repository starts from, so an entry written there ships as a stranger's history in every repository built afterward, describing work its readers never did. A change to what the payload ships is still notable — it is recorded here, where the readers are.
