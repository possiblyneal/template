...
- So do these paths, whether the change creates or modifies: any dotfile or dot-folder, anything under a `src/` directory, and anything directly in the repository root. Only the `src/` rule lives in this repository's `.claude/settings.json`, since it protects a boundary specific to this repo; the rest, and the `rm` rules above, come from the operator's global `~/.claude/settings.json`. `.claude/AGENTS.md` explains the split and why deletion needs the `rm` rules rather than a path rule.
